	...
	if(isset($_REQUEST['submit']) and check_admin_referer('watu_questions')) {
		...
		if($action == 'edit'){ //Update goes here
			...
			$wpdb->query($wpdb->prepare("UPDATE ".WATU_QUESTIONS." 
				SET question=%s, answer_type=%s, is_required=%d, feedback=%s, is_inactive=%d 
				WHERE ID=%d", $content, $answer_type, $is_required, 
				$feedback, $is_inactive, $question_id));
			...
			$sql = $wpdb->prepare("INSERT INTO ".WATU_QUESTIONS." (exam_id, question, answer_type, is_required, feedback, sort_order, is_inactive) 
			VALUES(%d, %s, %s, %d, %s, %d, %d)", $quiz_id, $content, $answer_type, 
				$is_required, $feedback, $sort_order, $is_inactive);
...
function watu_import_question($data, $exam_id) {
	...
	$sort_order = $wpdb->get_var($wpdb->prepare("SELECT MAX(sort_order) FROM ".WATU_QUESTIONS."
			WHERE exam_id=%d", $vexam_id));
	$sort_order++;
	
   $sql = $wpdb->prepare("INSERT INTO ".WATU_QUESTIONS." SET exam_id=%d, question=%s, answer_type=%s, sort_order=%d",
		$exam_id, watu_strip_tags($data[0]), sanitize_text_field($data[1]), $sort_order);
	$wpdb->query($sql);
	$qid = $wpdb->insert_id;	
     ...
     foreach($data as $cnt=>$d) {			  			
		...
		if(sizeof($answers)) { $wpdb->query("INSERT INTO ".WATU_ANSWERS." (question_id,answer,correct,point, sort_order) 
			VALUES $values_sql"); }	
}
