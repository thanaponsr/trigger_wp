...
function watu_takings($in_shortcode = false, $atts = null) {
	...
	if(!empty($_GET['watu_export'])) {
		...
		$rows[]=__('User or IP Address', 'watu').$delim.__('Date', 'watu').$delim.__('Points', 'watu').$delim.__('% Correct Answers', 'watu').
		...
		foreach($takings as $taking) {
			$row = ($taking->user_id ? $taking->user_login : $taking->ip).$delim.$quote.date(get_option('date_format'), strtotime($taking->date)).$quote.$delim.
